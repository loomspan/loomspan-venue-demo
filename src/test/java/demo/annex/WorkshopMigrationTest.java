...
{
    @Test
    void upgradesExistingRoomBookingsWithoutResettingThem() throws Exception
    {
        ...
        try (var c = DriverManager.getConnection(url, "sa", ""); var s = c.createStatement())
        {
            try (var rows = s.executeQuery("SELECT event_type,series_id,revision_number FROM event_request WHERE id='old-event'"))
            {
                assertThat(rows.next()).isTrue();
                assertThat(rows.getString(1)).isEqualTo("MEETING");
                assertThat(rows.getString(2)).isEqualTo("old-event");
                assertThat(rows.getInt(3)).isEqualTo(1);
            }
            try (var rows = s.executeQuery("SELECT COUNT(*) FROM resource_reservation WHERE booking_id='old-booking' AND resource_id='ROOM-C'"))
            {
                rows.next();
                assertThat(rows.getInt(1)).isEqualTo(1);
            }
            try (var rows = s.executeQuery("SELECT COUNT(*) FROM booking"))
            {
                rows.next();
